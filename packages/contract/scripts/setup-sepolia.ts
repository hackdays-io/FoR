/**
 * Sepolia テスト環境セットアップスクリプト
 *
 * デプロイ済みの FoRToken / RouterFactory から Router アドレスを算出し、
 * AllowList の設定と Indexer の設定ファイル更新を行う。
 *
 * 前提: deploy:sepolia:FoRToken, deploy:sepolia:RouterFactory, deploy:sepolia:Router が完了していること
 *
 * Usage: pnpm setup:sepolia
 */
...
const deployedPath = resolve(
  import.meta.dirname,
  `../ignition/deployments/chain-${SEPOLIA_CHAIN_ID}/deployed_addresses.json`,
);
...
if (!routerCode || routerCode === "0x") {
  console.error(
    ...
    "\nPlease run deploy:sepolia:Router first.",
  );
  process.exit(1);
}
...
Next steps:
  1. Update packages/frontend/app/lib/contracts.ts:
       11155111: { forToken: "${forTokenAddress}", router: "${routerAddress}" }
  2. Deploy subgraph: cd packages/indexer && pnpm deploy:sepolia
  3. Set frontend env: VITE_CHAIN_ID=11155111, VITE_SUBGRAPH_URL, VITE_ALCHEMY_KEY, VITE_PIMLICO_API_KEY
`);
